...
title: |
  ...
  \bigskip
  <center> **Association of Hateful Tweets and Hate Crimes in New York City** </center>
  \begin{center}
  \bigskip
  \bigskip
  \bigskip
...
Many of us spend several hours per week in social media platforms such as Facebook, Twitter, and Instagram to name a few. Although there are many advantages that social media provides e.g. serving as the fifth estate of power, a place to express opinions and such, it has also become a platform for someone to spread hate, cyberbullying, harassments and so on. Messages of hatred and spread of violence could have a high potential for influencing and motivating someone to commit a crime. According to CNN ([\textcolor{cyan}{Studley \& Tucker, 2022}](https://www.cnn.com/2022/04/16/us/hate-crimes-rise-in-new-york-city)), hate crimes in New York City have increased 76% in the first quarter of 2022 compared to last year. Many social media companies such as Facebook, have started to control the contents being posted by their users. Because there is a big concern in our society that offensive speech can result in more crimes and eventually become a serious threat to social, political, and cultural stability, we see the need to find a solution. 
...
$$
{Y}_{ij} \sim Binomial(n_{ij},p_{ij}) \tag{1}\\
$$

where ${Y}_{ij}$ is the observed number of hate crime. $n_{ij}$ is the population of potential hate crimes occurred at i-th location in the j-th period of time. $p_{ij}$ is the hate crime rate occurred at i-th location in the j-th period of time. By taking into the spatial and spatial-temporal correlation into account, model 1 above can be written as follows:

$$
logit(p_{ij}) = \alpha + \beta X_{i} + u_{i} + s_{i} + \gamma t_{j} + \sigma_{i} t_{j} \tag{2}\\
$$
...
$$
{Y}_{ij} \sim Poisson(E_{ij}\theta_{ij}) \tag{3}\\
$$
...
$$
log(\theta_{ij}) = \alpha + \beta X_{i} + u_{i} + s_{i} + \gamma t_{j} + \sigma_{i} t_{j} \tag{4}\\
$$

The terms which are used in both model 2 and 4 are explained here. $\alpha$ is the intercept, $\beta$ is the coefficient of covariate X at i-th location. $\beta X_{i}$ is the fixed effect of the model. $u_{i} + s_{i}$ is the area random effect. $\gamma t_{j}$ represents the purely temporal term, and its coefficient, $\gamma$. $\sigma_{i} t_{j}$ represents the space and time interaction term.
...
The crime data described as harassment and assault are considered as hate crimes and tweets data which are negative sentiments are selected for analysis in this study. 920 thousand out of 1.2 million of tweets are identified as negative attitude tweets. Many of these negative tweets contain key terms commonly used in hateful messages. In the figure 2, it shows the top 100 most frequent mentioned words.   

![Word clouds of top 100 most often mentioned words in the negative attitude tweets]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\wordcloud.png")

From the observed hate crime rate data, we found that these minority of groups, black, other races and female, are facing higher chance of being harassed and assaulted in most of the boroughs. The figure 3 and 4 below show these trends and particularly Bronx borough has the highest hate crime rate in both race and sex compared to other boroughs.

![Total hate crime by race for each borough between 2019 and 2021]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Hate Crime By Race.png"){width=80%}

$$\\[0.5in]$$

![Total hate crime by sex for each borough between 2019 and 2021]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Hate Crime By Sex.png"){width=80%}

\newpage

The observed hate crime rate data and detected negative attitude tweets from year 2019 to 2021 are analyzed. In the figure 5, we can see the trend of hate crimes is pretty stable over the 3 years period with slightly downward in 2020 and then increase again in 2021. In contrast, the negative attitude tweets drop sharply, nearly half compared to previous periods, after July 2020.

![Monthly count of crime and negative attitude tweets between 2019 and 2021]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Monthly Count Crime and Hateful Tweets.png")

\newpage

Now, if we put hate crimes and hateful tweets count together and compare for each borough as shown in the figure 6 and 7, Manhattan has the higher number of hate crime and hateful tweets followed by Bronx, Brooklyn, Queens, and Staten Island.

![Total hate crime and hateful tweets for each borough between 2019 and 2021]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Total Hate Crime vs Total Hateful Tweets by Borough.png"){width=80%}

$$\\[0.5in]$$

![Bivariate chloropleth map for total hate crime and hateful tweets in 2019]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Hate Crime vs Hateful Tweets 2019 Chloropleth Map.png"){width=70%}

\newpage

Bayesian inference approximation method, INLA (Integrated Nested Laplace Approximation) package ([\textcolor{cyan}{Rue et al, 2009}](https://rss.onlinelibrary.wiley.com/doi/10.1111/j.1467-9868.2008.00700.x)), is used to test the hypothesis of whether hateful tweets covariate is statistically significant correlated to the hate crimes and developing trend of relative risk of hate crime occurred in 5 boroughs. 
...
![Performance evaluation for Poisson and Binomial distribution models using DIC, WAIC, and CPO metrics]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Evaluation Table.png"){width=80%}

\newpage

From the analysis results as shown in the figure 9, we have observed that the hateful tweets covariate and time id variables which coefficient at 95% confidence interval contain zero for both models. This means uncertainty whether both of these variables have effect on contributing to hate crimes and implies they could have a positive or negative effect on the hate crime happening. Table 2 and 3 shows the coefficient at 95% confidence interval for Poisson models.

![Posterior distribution of hateful tweets covariate and time id variables coefficient at 95% confidence intervals]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Posterior Distribution of HT and TM Coefficient CI.png"){width=80%}

$$\\[0.3in]$$

![Fixed Effects Coefficient at 95% confidence intervals for Poisson model]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Fixed Effects Coefficient CI.png"){width=80%}

![Random Effects Coefficient at 95% confidence intervals for Poisson model]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Random Effects Coefficient CI.png")

\newpage

In addition to verifying the fixed variables' statistical significant, average posteriors of relative risks in each year for the 5 boroughs are computed and developing trends are showed in the figure 12. From the figure, we can see that the relative risk for Manhattan and Bronx has no change over the 3 years period. Because the relative risk of Bronx is much greater than 1, hate crime is more likely to occur in this area than its neighborhoods. The relative risk of Queens and Staten Island have become lower from 2019 to 2021 but Brooklyn has increased. This implies that Brooklyn areas post higher risk in hate crime occurring compared to other 4 boroughs over the 3 years period.

![Average posteriors of relative risks in each year for the 5 boroughs]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Relative Risk Each Borough.png"){width=80%}
...
The analysis results in above have verified that our hypothesis of hateful tweets covariate has no relationship with the hate crimes occurred since its coefficient is statistically insignificant. This matches with the result found in these ([\textcolor{cyan}{Williams et al., 2019}](https://academic.oup.com/bjc/article/60/1/93/5537169); [\textcolor{cyan}{Curiel et al., 2020}](https://www.nature.com/articles/s41599-020-0430-7#Sec8)) studies where there is no association between online hate speech to the hate crimes. But then the observed crime data have shown that hate crime did happen mostly on minority groups especially black, other races, and female.
...
Nowadays, social media plays a critical role in influencing people perception on everything we see, hear, and read in daily. The goal of this study is to find out if hateful tweets exist in Twitter has any association with the hate crimes in the New York City areas and whether minority groups are mostly the victim. Although we found that hateful tweets are not statistically significant correlate with hate crimes, but based on the observed NYPD crime data we do see minority groups which race is black, other, and female are mostly the victim for the hate crimes. For sure that there are room for improvements in this study. Instead of focusing the study in New York City areas, we can expand the scope to the whole New York state and conduct analysis at the county level since most tweets don't share their exact location. This will make the spatial random effect more effective when it has many measured levels. Random effects variance estimates will be unstable if the grouping variable has too few levels. For future research, we can incorporate an unsupervised learning technique such as topic modeling using LDA (Latent Dirichlet Allocation) to further filter out non-hateful tweets but this will require a computer which have a much bigger RAM and better GPU than regular laptop or desktop to perform matrix factorization on dataset size, 920k rows of text in our case. Also, we can leverage the same methodology used in this study to do hate crime prediction for each location. 
...
26. 2020 Census Data. NYC Planning. Available online: https://www.nyc.gov/site/planning/planning-level/nyc-population/2020-census.page##2020-census-results (accessed on 29 October 2022).

27. Moraga P. (2019, November 25). *Geospatial Health Data: Modeling and Visualization with R-INLA and Shiny*. Chapman & Hall/CRC Biostatistics Series.  Retrieved from https://www.paulamoraga.com/book-geospatial/sec-arealdataexamplest.html#model-2.

28. NYCdata. Baruch College. Available online: https://www.baruch.cuny.edu/nycdata/population-geography/age_distribution.htm (accessed on 24 November 2022),

29. Hu et al. (2018, October 31). *Urban crime prediction based on spatio-temporal Bayesian model*. National Library of Medicine.  Retrieved from https://ncbi.nlm.nih.gov/pmc/articles/PMC6209226/.

30. Rue et al. (2009, April 6). *Approximate Bayesian inference for latent Gaussian models by using integrated nested Laplace approximations*. Journal of the Royal Statistical Society. Retrieved from https://rss.onlinelibrary.wiley.com/doi/10.1111/j.1467-9868.2008.00700.x.

31. Besag et al. (1991). *Bayesian image restoration, with two applications in spatial statistics*. Springer. Retrieved from https://link.springer.com/article/10.1007/BF00116466.

32. Kim et al. (2018). *SpatialEpi: Methods and Data for Spatial Epidemiology*. Github. Retrieved from https://github.com/rudeboybert/SpatialEpi.

33. Howell D. (2003, January 3). *The Poisson and Binomial Distributions*. University of Vermont. Retrieved from https://www.uvm.edu/~statdhtx/StatPages/More_Stuff/PoissonBinomial/PoissonBinom.html.

34. Studley L. & Tucker E. (2022, April 16). *Hate crimes in New York City are up 76% this year compared to the same period in 2021*. CNN. Retrieved from https://www.cnn.com/2022/04/16/us/hate-crimes-rise-in-new-york-city.
