...
We'll use a sentiment analysis approach to annotate tweets into positive, negative, and neutral sentiments, to exclude non-hate tweets. We consider tweets which have negative score are highly related to hate speech. All identified hate tweets will then be mapped to the crimes reported by using date and borough for spatial and temporal analysis. Below is the architecture of the proposed methodology.
...
Our scope is to analyze the tweet's text created in the five boroughs of New York City: Manhattan, Brooklyn, Queens, Bronx, and Staten Island. But some tweets may have videos embedded and that's out of our capability to analyze the video's content. Only 32,224 tweets out of the 1.2 million tweets have geo-coordinates available; thus, we may not be able to fully perform spatial analysis at the zip code level. Another limitation to our study is that a simple binary classification of raw online posts into hate speech or non-hate speech is unlikely to be reliable compared to human moderation. With that being said, some of negative attitude tweets we get from sentiment analysis may not be 100% hate speech but could be expression of unhappiness.  
...
# Filter rows have geo coordinates
tweets_negative_zip <- tweets_negative %>% 
  filter(coordinates!="NULL")

# Function to get zip code from latitude and longitude
search_zip <- function(latitude, longitude) {
...
tweets_negative_zip$zipcode <- future_mapply(search_zip, 
                                             tweets_negative_zip$latitude, 
                                             tweets_negative_zip$longitude)

# Count tweets by year, month and zipcode
tweets_count_zip <- tweets_negative_zip %>% 
  mutate_at(vars(created_at), funs(year, week)) %>% 
  group_by(year, week, zipcode) %>% 
  summarise(total = n())
