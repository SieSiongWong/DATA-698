...
tweets_count_boro$name <- "tweets"
crime_count_boro$name <- "crime"
rbind(tweets_count_boro, crime_count_boro) %>% 
  ...
  ggtitle("Monthly Total Count from 2019 - 2021")
...
hate_crime %>% 
  ...
  ggplot(aes(x = boro_nm, y = total, fill = reorder(vic_race, -total))) + 
  geom_col(position = position_dodge2(reverse = TRUE)) + 
  theme(legend.title=element_blank(), 
        legend.position="bottom", 
        axis.title.x=element_blank(), 
        plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Total Hate Crime Victim By Race Across Each Borough (2019 - 2021)") + 
...
hate_crime %>% group_by(boro_nm, vic_sex) %>% summarise(total=n()) %>% 
  ggplot(aes(x = boro_nm, y = total, fill = reorder(vic_sex, -total))) + 
  geom_col(position = position_dodge2(reverse = TRUE)) + 
  theme(legend.title=element_blank(), 
        legend.position="bottom", 
        axis.title.x=element_blank(), 
        plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Total Hate Crime Victim By Sex Across Each Borough (2019 - 2021)") + 
  ylab("Count")
...
hate_crime %>% group_by(boro_nm, vic_age_group) %>% summarise(total=n()) %>% 
  ...
  theme(legend.title=element_blank(), 
        ...
        axis.title.x=element_blank(), 
        plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Total Hate Crime By Victim Age Group Across Each Borough (2019 - 2021)") + 
  ylab("Count")
...
crime_tweets_combine_boro <- gather(crime_tweets_combine_boro, condition, total, total_crime, total_hate_crime, total_hate_tweets, factor_key=TRUE)

ggplot(crime_tweets_combine_boro, aes(x = boro_nm, y = total, fill = condition)) + 
  geom_col(position = position_dodge2(reverse = TRUE)) + 
  theme(legend.title=element_blank(), 
        legend.position="bottom", 
        axis.title.x=element_blank(), 
        plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Total Count for Each Borough (2019 - 2021)") + 
  ylab("Total Count Per 100,000 Population")


# Chloropleth map for year 2019, 2020, and 2021
...
# Function to plot chloropleth map for hate crime vs hate tweets
...
map <- ggplot() + 
  ...
  labs(
    subtitle = paste0(year, " Hate Crime & Hate Tweets in NYC"),
  ) + 
  bi_theme()
...
# Function to plot chloropleth map for relative risk data from poisson distribution model
plot_chloro_map_rr_p <- function(df, year) {
...
data <- df %>% 
  ...
  rename(borough_name = boro_nm) %>% 
  mutate(borough_name = str_to_title(borough_name)) %>% 
  select(borough_name, total_hate_crime, avg_rr_p, avg_rr_b) %>% 
  left_join(boro_boudaries, ., by = c("borough_name")) %>% 
  bi_class(x = total_hate_crime, y = avg_rr_p, style = "quantile", dim = 3)

map <- ggplot() + 
  geom_sf(data = data, mapping = aes(fill = bi_class), color = "white", size = 0.1, show.legend = FALSE) +
  bi_scale_fill(pal = "GrPink", dim = 3) +
  labs(
    subtitle = paste0(year, " Hate Crime & Relative Risk by Poisson Distribution in NYC"),
  ) + 
  bi_theme()
...
# Plot all 3 years chloropleth maps
plot_chloro_map_rr_p(d2, 2019)
plot_chloro_map_rr_p(d2, 2020)
plot_chloro_map_rr_p(d2, 2021)

# Function to plot chloropleth map for relative risk data from binomial distribution model
plot_chloro_map_rr_b <- function(df, year) {
...
data <- df %>% 
  ...
  select(borough_name, total_hate_crime, avg_rr_p, avg_rr_b) %>% 
  left_join(boro_boudaries, ., by = c("borough_name")) %>% 
  bi_class(x = total_hate_crime, y = avg_rr_b, style = "quantile", dim = 3)

map <- ggplot() + 
  geom_sf(data = data, mapping = aes(fill = bi_class), color = "white", size = 0.1, show.legend = FALSE) +
  bi_scale_fill(pal = "GrPink", dim = 3) +
  labs(
    subtitle = paste0(year, " Hate Crime & Relative Risk by Binomial Distribution in NYC"),
  ) + 
  bi_theme()
...
# Plot all 3 years chloropleth maps
plot_chloro_map_rr_b(d2, 2019)
plot_chloro_map_rr_b(d2, 2020)
plot_chloro_map_rr_b(d2, 2021)
