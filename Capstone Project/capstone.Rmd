...
Popular metrics such as DIC (*deviance information criterion*), CPO (*conditional predictive ordinate*), and WAIC (*Watanabe–Akaike information criterion*), will be used as a performance measurement to compare and measure how good a model is.
...
![Figure 1: Word clouds of top 100 most often metnioned words in the negative attitude tweets]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\wordcloud.png")
...
Two types of Bayesian models are developed in this study. One model is based on Poisson distribution and another is based on Binomial distribution. For model performance evaluation, DIC, WAIC, CPO metrics are used. A smaller the value of these 3 metrics indicates a better prediction quality of the model. The table 2 shows Poisson model has better performance than Binomial model.

![Table 2: Performance evaluation for Poisson and Binomial distribution models using DIC, WAIC, and CPO metrics]("C:\\Users\\wongs34\\Documents\\School\\DATA 698\\Capstone\\Evaluation Table 2.png")

From the analysis results as shown in the figure 7, we have observed that the hateful tweets covariate and time id variables which coefficient at 95% confidence interval contain zero. This means uncertainty whether both of these variables have effect on contributing to hate crime and implies they could have a positive or negative effect on the hate crime happening.
...
In addition to verifying the fixed variables' statistical significant, average posteriors of relative risks in each year for the 5 boroughs are computed and developing trends are showed in the figure 8. From the figure, we can see that the relative risk for Manhattan and Bronx has no change over the 3 years period. The relative risk of Queens and Staten Island have become lower from 2019 to 2021 but Brooklyn has increased. This implies that Brooklyn areas post higher risk in hate crime occurring compared to other 4 boroughs.
...
The analysis results in above have verified that our hypothesis of hateful tweets covariate has no relationship with the hate crime occurred since its coefficient is statistically insignificant. This match with result mentioned in this ([\textcolor{cyan}{Williams et al., 2019}](https://academic.oup.com/bjc/article/60/1/93/5537169)) study where . 

But then observed crime data have proved that hate crime did happen mostly on minority groups especially black and other races and female.

Our research hypothesis is that higher offensive tweets from an area have strong correlation to the number of crimes committed in that area and minority groups are mostly the target. 

A popular spatial model is the Besag-York-Mollié (BYM) model (Besag, York, and Mollié 1991) which takes into account that data may be spatially correlated and observations in neighboring areas may be more similar than observations in areas that are farther away. This model includes a spatial random effect that smoothes the data according to a neighborhood structure, and an unstructured exchangeable component that models uncorrelated noise . In spatio-temporal settings where disease counts are observed over time, spatio-temporal models that account not only for spatial structure but also for temporal correlations and spatio-temporal interactions are used.

In the formula, we put hateful tweets as covariate because we think hateful tweets can explain the hate crime risk but of course other factors we have not measured we don't put in the model and we are going to model those factors using random effects. We are going to put spatial random effect, it's possible that two areas are close to each other they have similar risk. 


Although the fixed variables except the intercept, the spatial random effect components play a big role because we are observing that areas that are close to each other having similar risk. 

\newpage

# Conclusion

- collect longer period data from Twitter or from different social media
- introduce socio-economic variables
- instead of limiting to new york city areas, do for the whole new york state and analyze at county level
- 
...
annotate_figure(ggarrange(marginal_tw_p_plot + rremove("x.title"), 
                          marginal_tw_b_plot + rremove("x.title") + rremove("y.title"),
                          marginal_tm_p_plot, 
